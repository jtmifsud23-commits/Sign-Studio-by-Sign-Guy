...
const SUBMISSION_ENDPOINT = window.SIGN_GUY_SUBMISSION_ENDPOINT
  || (window.location.protocol.startsWith('http') ? '/api/submit-design' : '');
...
async function submitDesignRequest() {
  ...
  try {
    ...
    if (SUBMISSION_ENDPOINT) {
      await submitDesignToEndpoint({ subject, body, screenshots });
      setStatus('Submitted');
    ...
    if (navigator.canShare?.({ files }) && navigator.share) {
      ...
      setStatus('Shared');
      els.submitNote.textContent = `Share sheet opened with the logo and ${screenshots.length} visualizer screenshots. Send it to ${CONTACT_EMAIL}.`;
      return;
    }
    ...
    els.submitNote.textContent = 'A pre-addressed email draft was opened and the visualizer screenshots were downloaded for attachment.';
    setStatus('Email draft');
  } catch (error) {
    console.error(error);
    els.submitNote.textContent = SUBMISSION_ENDPOINT
      ? 'Could not send this submission. Check the submission endpoint and try again.'
      : 'Could not prepare the screenshots. Please try again after the preview finishes loading.';
...
async function submitDesignToEndpoint({ subject, body, screenshots }) {
